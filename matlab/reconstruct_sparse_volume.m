function reconstruct_sparse_volume()
    % Parameters
    ...
    volume = zeros(1952, 1952, num_slices);

    % Create the directory for saving reconstructed images if it doesn't exist
    if ~exist('./reconstructed_512_1_1', 'dir')
        mkdir('./reconstructed_512_1_1');
    end

    % Precompute the angles in radians and their sines and cosines
    theta_rad = linspace(0, 360, num_angles) * pi / 180;
    ...
    for slice_index = 1:step:height
        ...
        reconstructed_image_filename = sprintf('./reconstructed_512_1_1/reconstructed_512_fruit_%d_ramp.png', slice_index);
